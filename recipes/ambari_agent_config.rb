...
bash 'set_ambari-agent_file_permissions' do
  code <<-EOF
    chown -R #{node['hdp']['ambari-agent']['user']['name']}:root /etc/ambari-agent
    chown -R #{node['hdp']['ambari-agent']['user']['name']}:root /var/lib/ambari-agent
    chown -R #{node['hdp']['ambari-agent']['user']['name']}:root /var/log/ambari-agent
    chown -R #{node['hdp']['ambari-agent']['user']['name']}:root /var/run/ambari-agent
  EOF
end

# if truststore is enabled
if node['hdp']['ambari-agent']['config']['ssl_truststore_enabled'] == 'true'
  remote_file 'copy_/var/lib/ambari-agent/keys/keystore.jks' do 
    path '/var/lib/ambari-agent/keys/keystore.jks'
    source "file://#{node['hdp']['ambari-agent']['crypto']['truststore_jks']}"
    owner node['hdp']['ambari-agent']['user']['name']
    group 'root'
    mode 0600
  end
  remote_file 'copy_/var/lib/ambari-agent/keys/ca.crt' do 
    path '/var/lib/ambari-agent/keys/ca.crt'
    source "file://#{node['hdp']['ambari-agent']['crypto']['ca_cert']}"
    owner node['hdp']['ambari-agent']['user']['name']
    group 'root'
    mode 0600
  end
...
if node['hdp']['ambari-server']['config']['two_way_ssl'] == 'true'
  remote_file 'copy_/var/lib/ambari-agent/keys/https.crt' do 
    path "/var/lib/ambari-agent/keys/#{node['fqdn']}.crt"
    source "file://#{node['hdp']['ambari-agent']['crypto']['agent_cert']}"
    owner node['hdp']['ambari-agent']['user']['name']
    group 'root'
    mode 0600
  end
  remote_file 'copy_/var/lib/ambari-agent/keys/https.key' do 
    ...
    source "file://#{node['hdp']['ambari-agent']['crypto']['agent_key']}"
    owner node['hdp']['ambari-agent']['user']['name']
    group 'root'
    mode 0600
  end
...
template 'create_/etc/ambari-agent/conf/ambari-agent.ini' do
  ...
  source "ambari-agent.ini_#{node['hdp']['version']}.erb"
  owner node['hdp']['ambari-agent']['user']['name']
  group 'root'
